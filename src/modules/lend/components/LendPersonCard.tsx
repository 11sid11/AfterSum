...
export function LendPersonCard({ summary }: { summary: PersonSummary }) {
  ...
  return (
    <Link
      ...
      className="flex min-w-0 items-center gap-3 rounded-2xl border border-slate-200 bg-white p-3 shadow-sm transition-colors hover:border-brand-300 hover:bg-brand-50/50 dark:border-slate-800 dark:bg-slate-900 dark:hover:border-brand-700 dark:hover:bg-brand-900/20"
    >
      ...
      <div className="ml-auto flex shrink-0 flex-col items-end gap-0.5">
        <span className="text-xs font-medium text-slate-400" aria-label={`Status ${label}`}>{label}</span>
