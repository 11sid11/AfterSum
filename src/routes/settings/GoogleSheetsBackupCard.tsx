...
/** Manual, account-bound cloud backup. IndexedDB remains the canonical database. */
export function GoogleSheetsBackupCard() {
  ...
  const bindAndDiscover = async (nextAuth: GoogleAuthState, switching = false) => {
    ...
    if (!nextSession || !nextAuth.accountId) {
      throw new Error('Google authorization expired. Reconnect and try again.');
    }
    if (nextSession.accountId !== nextAuth.accountId) {
      throw new Error('Google account changed during authorization.');
    }

    const preferredId = switching ? undefined : settings.googleSpreadsheetId;
  ...
  const confirmAccountSwitch = async () => {
    const selected = pendingAccount;
    if (!selected?.accountId) return;

    setBusy('switch');
    try {
      await bindAndDiscover(selected, true);
      toast.show(
        `Google account switched to ${selected.email ?? 'the selected account'}. Local data was not changed.`,
      );
    } catch (error) {
      toast.show(errorMessage(error), { variant: 'error' });
    } finally {
  ...
  const backupNow = async () => {
    ...
    try {
      ...
      const spreadsheet = await ensureBackupSpreadsheet(
        currentSession,
  ...
  const restoreFromGoogle = async () => {
    ...
    try {
      assertGoogleAccount(currentSession, settings.googleAccountId);
      const spreadsheet = await findBackupSpreadsheet(
        currentSession,
        settings.googleSpreadsheetId,
      );
      ...
      downloadJson(
        safety,
        `aftersum-pre-cloud-restore-${new Date().toISOString().slice(0, 10)}.json`,
      );
            ...
            <p>
              ...
              <strong className="font-medium text-slate-700 dark:text-slate-200">
                {settings.googleAccountEmail ??
                  (settings.googleSyncEnabled ? 'Linked Google account' : 'Not connected')}
              </strong>
            </p>
            {settings.googleSyncEnabled && (
              <p>
                Authorization:{' '}
                {authorizedForBoundAccount ? 'Connected for this session' : 'Reconnect required'}
              </p>
            )}
            {cloud === null && <p>Cloud backup: none found in this account</p>}
            {cloud?.exportedAt && <p>Cloud backup: {formatTimestamp(cloud.exportedAt)}</p>}
            {!cloud?.exportedAt && sync?.lastSuccessfulSyncAt && (
              <p>Last backup: {formatTimestamp(sync.lastSuccessfulSyncAt)}</p>
            )}
            {sync?.dirty && (
              <p className="text-amber-600">Local changes are waiting to be backed up.</p>
            )}
          </div>
...
function errorMessage(error: unknown): string {
  if (typeof error === 'object' && error !== null && 'result' in error) {
    const result = (error as { result?: { error?: { message?: string } } }).result;
