  ...
  const onDelete = async () => {
    ...
    try {
      ...
      toast.show('Transaction deleted', {
        action: {
          label: 'Undo',
          onClick: async () => trackTransactionRepository.restore(transaction.id),
        },
      });
      navigate({ to: '/track' });
    } catch (err) {
      ...
      <Card>
        <div className="flex min-w-0 items-baseline justify-between gap-2">
          ...
          <p
            className={
              isExpense
                ? 'min-w-0 truncate text-2xl font-semibold text-rose-600 dark:text-rose-300'
                : 'min-w-0 truncate text-2xl font-semibold text-emerald-600 dark:text-emerald-300'
            }
          >
            <Money
        ...
        <p className="mt-1 text-xs text-slate-400">
          Created {formatHumanDateTime(transaction.createdAt)}
        </p>
      </Card>
